...
component_examples_of_left = '口(叶),亻(仁),扌(持),忄(惜),𤣩(球),彳(行),釒(銀),訁(語),飠(飲),⻖(阪),犭(狼),衤(被),礻(神),⺬(祇),⻊(路),冫(冷),氵(河),支(𠚽),木(機),糸,舌,辛(辡),甫,歹(𣩵),文(刘),龠,禾(秋),齒,雚,蒙,𠁣(門),𩰋(鬥),坐,了,同(𡜝),𫵖,子,孑(孫),斉,咅,仌,丁,亭,彦(顔),矢(知),㠯(𰀥),𦰩(難),巳(𠨎),己(𠨎),刃(𱐔),彡(須),立(䇃),堇(勤),人(从),矛(敄),尚(敞),十(卄),丰(邦),田(畼),矣(欸),豆(頭),豸(豹),羊(𦍏),甬(勈),日(明),昌(𣣘),巾(幅),目(眼),鹿(𪊺),或(𠜻),褱(𣀤),侯(鄇),垔(歅),産(剷),声(殸),方(放),秝(𥣸),止(此),𡰪(辟),夢(𪇓),委(魏),眔(𣣴),𲺋(毻),⺼(脈),月(朋),女(好),戸(戺),貝(販),豦(劇),育(𠃴),僉(劒),㐱(𩒉),翏(剹),隶(𠡫),賔(𭄗),買(𧶶),身(射),弓(弘),𧶠(𧸷),者(都),帛(𢄗),㬰(𣢧),壴(鼓),酉(配),學(斆),又(双)'

component_examples_of_right = '刂(劍),⻏(郭),卩(叩),攵(牧),攴,口(加),乚(礼),夌,垔(煙),豈,栗,倉,冥,彔,欠,𠃛(門),𩰊(鬥),㣊,菐,犬,丣,坐,與,舁,頃(傾),了,共(供),巽(撰),同(銅),子(㝀),齊,斉,咅,仌,互(坘),亟(極),丁,亭,彦(諺),矢(䀢),難(儺),㠯(佀),矣(俟),尹(伊),巳(𠨎),己(𠨎),刃(仞),彡(杉),頁(頃),立(位),冉(呥),厶(私),堇(謹),人(从),从(𪻐),杀(刹),𣏂(剎),矛(䋒),𠂉(𭤨),尚(倘),衣(挔),𧘇(𠇊),几(机),支(伎),十(什),丰(仹),卯(柳),田(畑),尺(択),豆(脰),呉(誤),辡(𡁈),羊(垟),𦉰(𤤡),亡(忙),舞(儛),甬(俑),孔(吼),日(𬽪),昌(唱),𠔼(㧇),巾(𪤷),布(怖),目(相),鹿(漉),修(㹋),或(域),褱(懷),咸(减),侯(喉),四(伵),産(嵼),文(伩),士(仕),声(𠴢),方(㕫),秝(𥕆),禾(秌),亦(㑊),辛(垶),夢(懜),委(倭),鬼(魏),戎(娀),戻(涙),夬(決),眔(𣗶),弔(伄),敫(激),放(𦜍),左(佐),𲺋(隋),月(明),衍(𢯼),曽(増),女(㚢),奴(伮),戸(䋆),豦(據),𠫓(育),育(𰕎),僉(險),曹(傮),㐱(珍),翏(僇),隶(埭),賔(𣩵),歹(㱛),買(嘪),身(㑗),弓(弜),弓(㧈),具(倶),冢(塚),凶(忷),𧶠(儥),者(渚),卉(𠱥),帛(綿),㬰(𣧴),壴(𠊪),酉(酒),畺(橿),學(㙾),又(双)'

component_examples_of_above = '亠,宀,冖(冠),⺮,艹(華),癶,𭼽,罒(睘),覀,⺷(義),屮(㞷),爫,彐,彑,夂(冬),文(产),䒑(屰),业(業),兴,加,卯,次,所,𣅀,立(音),㐭,𠆢,亼,亽,六,𫩠,八,⺜,𠀎,准,禾(季),𦥯(學),𦥯(覺),龹(券),𤇾(榮),𰃮(労),𫇦,髟(髪),冎(骨),𡨄(寒),𣦼,殸(聲),攸(悠),氶(丞),处,丣,一,𦥑,與,頃(𣻯),了,共(巷),龻,𰀉,灾,𦥔,吅,咅,亟,𦭝(蔑),難(臡),𠀐(貴),亞(惡),㇇(𡥀),𠨎(巽),刃(忍),彡(辵),攵(㣊),不(否),冉(𣅾),厶(弁),人(介),从(怂),吅(單),𱼀(䍃),矛(柔),巛(巢),𠂉(乞),尚(堂),衣(𧘉),几(殳),支(𥁈),十(古),丰(𣐇),卯(𨥫),田(界),尺(𪽗),𠂒(先),口(兄),羊(羴),龴(甬),亡(巟),甬(勇),子(孟),孔(𡵾),日(早),昌(㫯),𠔼(冡),同(𣑸),巾(𢁝),目(見),鹿(麋),修(𢟅),或(𢃤),咸(感),卄(共),四(𰉐),士(声),士(賣),秝(𬓶),止(歯),亦(変),辛(㖖),辡(𰺪),夢(𲀎),𦭝(夢),𡗗(春),彦(𲖐),弔(𪫥),敫(𣜥),左(𲺋),衍(愆),奴(努),厸(𠫞),隶(𨽶),炏(𤇾),𤇾(勞),公(兊),歹(𢁳),具(𭉦),凶(兇),𦣻(奡),者(煮),卉(桒),卉(尭),壴(喜),酉(尊),又(𠬟)'

component_examples_of_below = '⺗,灬,龰,夂(夏),夊,口(古),二,儿(兄),几(亢),了,子(字),旦,丂,𰆊,八,大,犬,𬺢(具),厶(去),厶(公),彡,難,菐,廾(卉),𪱙,丣,坐,一(丞),與,舁,頃(䔛),了,共(巽),巽(𦺈),同(𠀹),吅,互(𦬚),亟,丁,亭,屮(𡗡),矢(矣),難(𦍀),㠯(官),𪟊(寡),刃(𦬄),彡(㐱),立(笠),不(示),冉(再),堇(蓳),人(珡),从(𠅃),业(並),朩(杀),朮(𣏂),术(𣏂),吅(品),矛(罞),巛(𠮰),𠦒(華),禸(禺),衣(装),𧘇(衣),支(芰),十(早),丰(夆),卯(奅),田(畗),尺(𫁶),𠨎(𠨕),矣(𦮸),䒑(豆),𠂒(𭇆),豆(壴),呉(茣),羊(𦍒),龴(令),甬(𥦁),孔(芤),日(晶),昌(菖),巾(帀),布(希),目(盲),鹿(蔍),攸(峳),修(蓚),或(𦱂),咸(嵅),侯(篌),垔(𡨾),四(𣳉),厂(产),文(㞵),方(𬙙),秝(𩄞),止(正),禾(𦊜),辛(宰),罒(𦭝),委(萎),鬼(䰟),戎(茙),戻(䈆),夬(䆕),眔(𦵺),弔(弚),敫(𡫀),放(𭣺),左(𢀡),⺼(𲺋),衍(葕),女(姿),戸(𢨥),𠃜(声),貝(貴),貝(買),厸(厽),曹(𥲍),㐱(翏),翏(寥),炏(焱),公(𠕺),𮙸(賔),賔(𦿜),買(賣),弓(宆),躬(窮),具(𰩑),凶(㐫),𦣻(首),者(著),卉(奔),帛(𦰬),酉(酋),畺(薑),又(攴),双(叒),双(叕)'

component_examples_of_surround_from_upper_left = '厂(厚),𠂆(𠂢),𠂋(后),厃(危),疒(病),尸(屈),广(廣),戸(戻),虍(處),𬻉,𭤨(旗),倝(幹),产(彦),𠂇(右),麻(磨),鹿(麃),⺶,攸(修),厤(曆),𠩵(暦),耂(考),尹(君),巾(𢁟),方(房),𠃜(眉),歹(死)'

component_examples_of_surround_from_lower_left = '⻌(進),廴(建),走(起),麥,麦,風,支,爪,毛,夊,鼠,文,乙,𠃊,元,克,光,是,支(𭣗),尺(𡰰),乚(匕),鬼(魅)'

component_examples_of_surround_from_lower_right = '放(敖)'
...
OUTPUT_PATH = f'hybrid_{model_name}/{stage1_CID}+{stage1_CID}/tsv_pct100'
os.makedirs(OUTPUT_PATH, exist_ok=True)
TSV_OUTPUT_PATH = f'hybrid_{model_name}/{stage1_CID}+{stage1_CID}/tsv_pct100'
